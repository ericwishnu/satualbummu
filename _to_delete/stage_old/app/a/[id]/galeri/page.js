...
function publicUrl(path, download) {
...
export default function Gallery({ params }) {
  ...
  useEffect(() => {
    ...
    if (revealed && !loadedPhotos) loadPhotos()
  }, [revealed, loadedPhotos, albumId])
  ...
  async function downloadAll() {
    ...
    try {
      ...
      for (let i = 0; i < photos.length; i++) {
        const p = photos[i]
        const resp = await fetch(publicUrl(p.storage_path))
          ...
          <div className="grid">
            {photos.map((p) => (
              <div className="tile" key={p.id}>
                ...
                <img src={publicUrl(p.storage_path)} alt={p.uploader_name || 'foto'} loading="lazy" />
                <a className="dl" href={publicUrl(p.storage_path, true)}>Unduh</a>
              </div>
            ))}
          </div>
        </>
